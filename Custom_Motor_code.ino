...
void messageCb( const geometry_msgs::Twist& msg){
 ...
 w_r = 1.5+abs((speed_lin/wheel_rad) + ((speed_ang*wheel_sep)/(2.0*wheel_rad)));
 w_l = 1.5+abs((speed_lin/wheel_rad) - ((speed_ang*wheel_sep)/(2.0*wheel_rad)));
 
 //w_r_const = 2+(7.78/wheel_rad) + ((7.78*wheel_sep)/(2.0*wheel_rad));
 //w_l_const = 2+(7.78/wheel_rad) - ((7.78*wheel_sep)/(2.0*wheel_rad));
 ...
 rightMotorSpeed = 400;
 spinSpeed =350;
 //spinSpeed = ((leftMotorSpeed+rightMotorSpeed)/2<500)?((leftMotorSpeed+rightMotorSpeed)/2):500;
}
...
double p2= w_r*10;
  ...
  if(speed_lin>0 )//&& speed_ang==0)
   ...
   TurnRightTheBot();//SpinRightTheBot();

 else if((speed_lin>=0 && speed_ang>0)||(speed_lin<0 && speed_ang<0))
   TurnLeftTheBot();//SpinLeftTheBot();
